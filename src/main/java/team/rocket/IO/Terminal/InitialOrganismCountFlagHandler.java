...
/**
 * Handles terminal flags that pertain to initial organism Count. Will put an initial amount of Organisms onto the map.
 * Ex: --rabbit_count 6.
 *
 * @version 0.6.0
 * @since 0.3.0
		...
		for(String flag:flags) {
			...
			if (matchFound) {
				...
				if (totalOrganismsBeingAdded > tFRequest.getMap().getWidth() * tFRequest.getMap().getHeight()) {
					System.out.println("WARNING: You are attempting to place more organisms then the grid can hold, " +
